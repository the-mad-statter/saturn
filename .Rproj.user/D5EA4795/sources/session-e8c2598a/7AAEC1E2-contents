...
#' Saturn Execute
#'
#' @param session ssh connection created with \code{\link{sat_connect}}
#' @inheritParams ssh::ssh_exec_internal
#'
#' @return list containing exit status, buffered raw stdout, and buffered raw
#' stderr
#' @export
#'
#' @examples
...
#' session <- sat_connect()
#' sat_download(session, "~/target/*", tempdir())
#' }
#'
#' @seealso \code{\link[ssh]{scp}}
sat_download <- function(
    session,
    files,
    to = ".",
    verbose = FALSE) {
...
#' sat_upload(session, files, "~/target")
#' }
#'
#' @seealso \code{\link[ssh]{scp}}
sat_upload <- function(
...
#' \dontrun{
#' session <- sat_connect()
#' sat_disconnect(session)
#' }
#'
#' @seealso \code{\link[ssh]{ssh}}
sat_disconnect <- function(session) {
...
#' @param queue queue name (note, this will be auto redirected depending on
#' resource request)
...
#' @param ... additional arguments passed to \code{\link{sat_execute}} and
#' \code{\link{sat_upload}}
#'
#' @export
#'
#' @examples
...
#' job_name <- "my_job"
#' sat_job_init(job_name)
#' sat_job_upload(session, job_name)
#' }
sat_job_upload <- function(
    session,
    job_name,
    mk_spath = FALSE,
    ...) {
  ...
  r <- sat_execute(
    session,
    sprintf("if test -d %s; then echo \"exist\"; fi", .sjob$spath),
    ...
  )
  if (length(r$stdout) == 0) {
    if (mk_spath) {
      sat_execute(session, sprintf("mkdir -p %s", .sjob$spath))
    } else {
      stop(sprintf(
        "The remote directory /home/%s/%s does not exist.",
        .sjob$user,
        .sjob$spath
      ))
    }
  ...
  r <- sat_upload(session, job_name, .sjob$spath, ...)
}
...
#' @param session ssh connection created with \code{\link{sat_connect}}
#' @param job_name job name
#' @param ... additional arguments passed to \code{\link{sat_execute}},
#' \code{\link{sat_upload}}
#'
#' @export
#'
...
#' sat_job_init(job_name)
#' sat_job_upload(session, job_name)
#' sat_job_submit(session, job_name)
#' }
sat_job_submit <- function(
    session,
    job_name,
    ...) {
  load(file.path(job_name, ".sjob"))
  response <- sat_execute(
    session,
    c(
  ...
  r <- sat_upload(
    session,
    file.path(job_name, ".sjob"),
    file.path(.sjob$spath, .sjob$name),
    ...
  )
  invisible(r)
}
...
#' session <- sat_connect()
#' job_name <- "my_job"
#' sat_job_init(job_name, add_cron = TRUE)
#' sat_job_upload(session, job_name)
#' sat_job_schedule(session, job_name)
#' }
#'
#' @details \tabular{ll}{
...
#' crontab -a job_name.cron \tab schedule a job                 \cr
#' }
#'
#' @note check mail with mail command
sat_job_schedule <- function(
    session,
    job_name,
    ...) {
  load(file.path(job_name, ".sjob"))
...
#' sat_job_upload(session, job_name)
#' sat_job_submit(session, job_name)
#' sat_job_running(session, job_name)
#' }
sat_job_running <- function(
    session,
    job_name) {
  .sjob <- NULL
  load(file.path(job_name, ".sjob"))
  r <- sat_execute(session, sprintf("qstat %s", .sjob$id), FALSE)
...
#' @param session ssh connection created with \code{\link{sat_connect}}
#' @param job_name job name
#' @param ... additional arguments passed to \code{\link{sat_upload}}
#'
#' @export
#'
#' @examples
...
#' sat_job_submit(session, job_name)
#' sat_job_running(session, job_name)
#' sat_job_download(session, job_name)
#' }
sat_job_download <- function(
    session,
    job_name,
    ...) {
  .sjob <- NULL
  load(file.path(job_name, ".sjob"))
  remote_path <-
    ifelse(.sjob$spath == ".", job_name, file.path(.sjob$spath, job_name))
  sat_download(session, remote_path, ".", ...)
  save(.sjob, file = file.path(job_name, ".sjob"))
}
...
#' sat_job_running(session, job_name)
#' sat_job_download(session, job_name)
#' sat_job_remove(session, job_name)
#' sat_job_remove(session, job_name, "local")
#' }
sat_job_remove <-
  function(session,
           job_name,
           location = c("remote", "local"),
           ...) {
    .sjob <- NULL
    ...
    if (location == "remote") {
      load(file.path(job_name, ".sjob"))
      sat_execute(
        session, sprintf("rm -rf %s", file.path(.sjob$spath, job_name)), ...
      )
    } else {
...
#' \dontrun{
#' job_name <- "my_job"
#' sat_job_init(job_name, edit_script = FALSE)
...
#' job_name <- "my_job"
#' sat_job_init(job_name)
#' sat_job_edit_cron(job_name)
#' }
#'
#' @note The output of the cron command will be automatically sent to your
#' local email account. If you want to stop receiving these emails, you can add
...
#' -B        \tab list summary status of the job server \cr
#' }
sat_qstat <- function(
    session,
    args,
    ...) {
  sat_execute(session, sprintf("qstat %s", paste(args, collapse = " ")), ...)
}

#' Saturn pbsnodes
#'
#' @param session ssh connection created with \code{\link{sat_connect}}
#' @param ... additional arguments passed to \code{\link{sat_qstat}}
#'
...
#' \dontrun{
#' session <- sat_connect()
#' r <- sat_pbsnodes(session)
#' cat(rawToChar(r$stdout))
#' }
sat_pbsnodes <- function(
    session,
    ...) {
  r <- sat_execute(session, "pbsnodes", ...)
  rawToChar(r$stdout)
}
